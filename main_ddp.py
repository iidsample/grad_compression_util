...
try:
    ...
    s3_utils = s3_dummy
    print("Not able to initialize S3 functionalities")
...
def _get_compression_param(reducer_name, device, reducer_param):
    if reducer_name == "PowerSGD":
        reducer = gradient_reducers.RankKReducer(
            random_seed=42,
            device=device,
            timer=timer,
    ...
    if reducer_name == "SignSGD":
        reducer = gradient_reducers.SignSGDwithMajorityVoteReducer(
            random_seed=42, device=device, timer=timer
        )
    if reducer_name == "Topk":
        reducer = gradient_reducers.GlobalTopKReducer(
            random_seed=42, device=device, timer=timer, compression=reducer_param
        )

    if reducer_name == "MSTopK":
        reducer = gradient_reducers.MsTopKReducer(
            random_seed=42, device=device, timer=timer, k=reducer_param
        )
...
def main_resnet50(args, bsize):
    ...
    for batch_idx in range(100):
        ...
        if batch_idx == 30:
            ...
            file_name = "resnet50_out_file_{}_bsize_{}.json".format(global_rank, bsize)
...
def main_resnet101_single(args, bsize):
    ...
    for batch_idx in range(100):
        ...
        if batch_idx == 30:
            ...
            data_dict["args"] = args.__str__()
            data_dict["timing_log"] = time_list
            file_name = "resnet101_out_file_{}_bsize_{}.json".format(global_rank, bsize)
            with open(file_name, "w") as fout:
                json.dump(data_dict, fout)
            file_uploader.push_file(
                file_name, "{}/{}".format(args.s3_prefix, file_name)
...
def powersgd_single_call(args, psgd_rank, bsize, network_name):
    ...
    state = PowerSGD.PowerSGDState(
        process_group=None, matrix_approximation_rank=psgd_rank, start_powerSGD_iter=3
    )
    ...
    for batch_idx in range(100):
        ...
        if batch_idx == 30:
            ...
            data_dict["args"] = args.__str__()
            data_dict["timing_log"] = time_list
            file_name = "{}_powersgd_rank_{}_out_file_{}_batch_size_{}.json".format(
                network_name, psgd_rank, global_rank, bsize
            )
            with open(file_name, "w") as fout:
                json.dump(data_dict, fout)
            file_uploader.push_file(
...
def encode_decode_signsgd(state, bucket):
    ...
    fut = dist.all_gather(
        copy_bits, bits, group=dist.group.WORLD, async_op=True
    ).get_future()
...
def signsgd_single_call_reducer(args, bsize, network_name):
    ...
    for batch_idx in range(100):
        ...
        if batch_idx == 50:
            ...
            data_dict["args"] = args.__str__()
            data_dict["timing_log"] = time_list
            file_name = "{}_signsgd_overlap_out_file_{}_batch_size_{}.json".format(
...
def encode_decode(state, bucket):
    ...
    out_list = [
        torch.zeros_like(kai, device=kai.device, dtype=kai.dtype)
    ...
    dist.all_gather(out_list, kai, group=group_to_use, async_op=True)

    fut = dist.all_gather(out_list, kai, group=group_to_use, async_op=True).get_future()

    def decode(fut):
        agg_tensor = fut.value()[0]
        fut_tensor = grad_1d
        out_tensor = torch.zeros_like(
            fut_tensor, device=tensor.device, dtype=tensor.dtype
        )
...
def mstopk_single_call_reducer(args, topk_k, bsize, network_name):
    ...
    for batch_idx in range(100):
        ...
        if batch_idx == 30:
            ...
            data_dict["args"] = args.__str__()
            data_dict["timing_log"] = time_list
            file_name = "{}_mstopk_overlap_k_{}_out_file_{}_batch_size_{}.json".format(
                network_name, topk_k, global_rank, bsize
            )
            with open(file_name, "w") as fout:
                json.dump(data_dict, fout)
            file_uploader.push_file(
...
def mstopk_serial(args, topk_k, bsize, network_name):
    ...
    for batch_idx in range(100):
        ...
        if batch_idx == 30:
            ...
            data_dict["args"] = args.__str__()
            data_dict["timing_log"] = time_list
            file_name = "{}_mstopk_serial_k_{}_out_file_{}_batch_size_{}.json".format(
                network_name, topk_k, global_rank, bsize
            )
            with open(file_name, "w") as fout:
                json.dump(data_dict, fout)
...
def powersgd_resnet101(args, psgd_rank, bsize):
    print("Starting Resnet 101 PowerSGD")
    assigned_device = "cuda:{}".format(args.local_rank)
    torch.cuda.set_device(args.local_rank)
    global_rank = args.node_rank * 4 + args.local_rank
    model = models.__dict__["resnet101"]()
    ...
    for batch_idx in range(100):
        ...
        if batch_idx == 30:
            ...
            data_dict["timing_log"] = time_list
            file_name = (
                "resnet101_powersgd_rank_{}_out_file_{}_batch_size_{}.json".format(
                    psgd_rank, global_rank, bsize
                )
            )
            with open(file_name, "w") as fout:
                json.dump(data_dict, fout)
...
def signsgd_resnet101(args, bsize):
    ...
    for batch_idx in range(100):
        ...
        if batch_idx == 50:
            ...
            data_dict["args"] = args.__str__()
            data_dict["timing_log"] = time_list
            file_name = "resnet101_signsgd_serial_out_file_{}.json".format(global_rank)
            with open(file_name, "w") as fout:
                json.dump(data_dict, fout)
            file_uploader.push_file(
                file_name, "{}/{}".format(args.s3_prefix, file_name)
...
def signsgd_resnet50(args, bsize):
    ...
    for batch_idx in range(100):
        ...
        if batch_idx == 50:
            ...
            data_dict["args"] = args.__str__()
            data_dict["timing_log"] = time_list
            file_name = "resnet50_signsgd_serial_out_file_{}.json".format(global_rank)
            with open(file_name, "w") as fout:
                json.dump(data_dict, fout)
            file_uploader.push_file(
                file_name, "{}/{}".format(args.s3_prefix, file_name)
