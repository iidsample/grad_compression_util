...
def run_large_scale():
    ...
    run_args = [
        {
            "cmd": "git clone https://github.com/iidsample/grad_compression_util.git && cd grad_compression_util && bash run_ddp.sh {} {} 64 /home/ubuntu/imagenet_data trial {} resnets_bsize_64 ".format(
                private_ip[0], i, len(private_ip)
            )
        }
        for i in range(launch_cfg["instance_count"])
    ]
    print(run_args)
