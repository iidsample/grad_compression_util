...
class SignAndNormReducer(Reducer):
    ...
    def reduce(self, grad_in, grad_out, memory_out):
        ...
        bits_communicated += n_bits(
            my_bits
        )  # for the norm vector, being optimistic here
        ...
        with self.timer("reduce.average", verbosity=2):
            ...
            for their_flatsigns, their_norms in zip(flatsigns, norms):
                ...
                for sign, out, norm in zip(flatgrad, grad_out, their_norms):
                    out.data.add_(
    ...
    def reduce(self, grad_in, grad_out, memory_out):
        ...
        with self.timer("reduce.flatpack", verbosity=2):
            ...
            flat_positions = torch.empty(
                flatgrad_size, device=self.device, dtype=torch.int
            )
        # end_time.record()
        # torch.cuda.synchronize()
        ...
        with self.timer("reduce.gather", verbosity=2):
            if self.n_workers > 1:
                worker_values = [
                    torch.empty_like(flat_values) for i in range(self.n_workers)
                ]
                worker_positions = [
                    torch.empty_like(flat_positions) for i in range(self.n_workers)
                ]
    ...
    def reduce(self, grad_in, grad_out, memory_out):
        ...
        with self.timer("reduce.set_memory", verbosity=2):
            for tensor, mem, start, end in zip(
                ...
                local_positions = (
                    positions[(positions >= start) & (positions < end)] - start
                )
                mem.data[:] = tensor
        ...
        with self.timer("reduce.reduce", verbosity=2):
            if self.n_workers > 1:
                worker_values = [
                    torch.empty_like(values) for i in range(self.n_workers)
                ]
                worker_positions = [
                    torch.empty_like(positions) for i in range(self.n_workers)
                ]
...
class UniformRandomSparseBlockReducer(Reducer):
    ...
    def reduce(self, grad_in, grad_out, memory_out):
        ...
        with self.timer("reduce.memory", verbosity=2):
            for tensor, mem, start_idx, block_size in zip(
                grad_in, memory_out, start_idx_list, block_sizes
            ):
                end_idx = min(start_idx + block_size, tensor.nelement())
                rest = block_size - (end_idx - start_idx)
        ...
        with self.timer("reduce.combine", verbosity=2):
            for tensor, out, start_idx, block_size, values in zip(
                grad_in, grad_out, start_idx_list, block_sizes, flat_values
            ):
                end_idx = min(start_idx + block_size, tensor.nelement())
                rest = block_size - (end_idx - start_idx)
                out.data.zero_()
...
class UniformRandomSparseReducer(Reducer):
    ...
    def reduce(self, grad_in, grad_out, memory_out):
        ...
        with self.timer("reduce.combine", verbosity=2):
            for tensor, out, values, indices in zip(
                grad_in, grad_out, flat_values, indices_list
            ):
                out.data.zero_()
            ...
            for tensor in grad_in:
                ...
                if block_size == tensor.nelement():
                    ...
                    indices = self.rng.choice(
                        tensor.nelement(), block_size, replace=False
                    )
                indices_list.append(indices)
                values = tensor.view(-1)[indices]
                values_list.append(values)
...
class RankKReducer(Reducer):
    ...
    def reduce(self, grad_in, grad_out, memory_out):
        ...
        with self.timer("reduce.rank1.pack", verbosity=2):
            rank1_tensor_list = TensorBuffer(
...
class MsTopKReducer(Reducer):
    ...
    def reduce(self, grad_in, grad_out):
        ...
        with torch.cuda.stream(s):
            ...
            index_list = [
                torch.zeros_like(l, device=l.device, dtype=l.dtype)
...
class DGCReducer(Reducer):
    """
    Implement DGC reducer
    """

    def __init__(self, random_seed, device, timer, compression_param=0.1):
        super.__init__(random_seed, device, timer)
        self.compression_param = 0.1
...
class ExactReducer(Reducer):
    def reduce(self, grad_in, grad_out, memory_out):
        ...
        with self.timer("reduce.reduce", verbosity=2):
            bits_communicated = reduce_mean_list(
                self.device, list_in, list_out, self.timer
            )
